# Aria - Creative Companion: MCP Server Integration Guide

This guide explains how to configure and utilize external tool providers through the Model Context Protocol (MCP) with Aria - Creative Companion. This allows Aria - Creative Companion to leverage tools hosted by separate local or remote MCP-compliant servers.

## What are MCP Servers?

MCP servers are external processes or services that expose a set of tools that Aria - Creative Companion can use. Aria - Creative Companion acts as an MCP *client*, consuming tools made available by these servers. The integration supports three main types of MCP servers:

1.  **Local Stdio Servers**: These are typically local executables that Aria - Creative Companion communicates with via standard input/output (stdio).
2.  **Remote SSE Servers**: These are servers, often accessible over a network, that Aria - Creative Companion communicates with using Server-Sent Events (SSE), usually over HTTP/S.
...
## How Aria - Creative Companion Consumes MCP Tools

Aria - Creative Companion discovers and integrates MCP tools dynamically:

1.  **Configuration**: You define the MCP servers Aria - Creative Companion should connect to in its configuration. The primary way to do this is through the Aria - Creative Companion settings UI.
2.  **Saving Settings**: When you save your settings via the UI, Aria - Creative Companion updates the `tmp/settings.json` file, specifically the `"mcp_servers"` key.
3.  **Automatic Installation (on Restart)**: After saving your settings and restarting Aria - Creative Companion, the system will attempt to automatically install any MCP server packages defined with `command: "npx"` and the `--package` argument in their configuration (this process is managed by `initialize.py`). You can monitor the application logs (e.g., Docker logs) for details on this installation attempt.
4.  **Tool Discovery**: Upon initialization (or when settings are updated), Aria - Creative Companion connects to each configured and enabled MCP server and queries it for the list of available tools, their descriptions, and expected parameters.
...
6.  **Tool Invocation**: When the LLM decides to use an MCP tool, Aria - Creative Companion's `process_tools` method (handled by `mcp_handler.py`) identifies it as an MCP tool and routes the request to the appropriate `MCPConfig` helper, which then communicates with the designated MCP server to execute the tool.

## Configuration

### Configuration File & Method

The primary method for configuring MCP servers is through **Aria - Creative Companion's settings UI**.
...
*   **Default Value**: If `tmp/settings.json` does not exist, or if it exists but does not contain the `"mcp_servers"` key, Aria - Creative Companion will use a default value of `""` (an empty string), meaning no MCP servers are configured.
...
*   **Updating**: As mentioned, the recommended way to set or update this value is through Aria - Creative Companion's settings UI.
*   **For Existing `settings.json` Files (After an Upgrade)**: If you have an existing `tmp/settings.json` from a version of Aria - Creative Companion prior to MCP server support, the `"mcp_servers"` key will likely be missing. To add this key:
    1.  Ensure you are running a version of Aria - Creative Companion that includes MCP server support.
    2.  Run Aria - Creative Companion and open its settings UI.
...
*   `"disabled"`: A boolean (`true` or `false`). If `true`, Aria - Creative Companion will ignore this server configuration.
...
Once configured, successfully installed (if applicable, e.g., for `npx` based servers), and discovered by Aria - Creative Companion:
