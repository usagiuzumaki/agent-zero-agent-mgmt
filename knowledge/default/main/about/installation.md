...
- This directory will contain all your Aria - AI Creative Companion files, like the legacy root folder structure:
  ...
  - `.env` - Your API keys
  - `settings.json` - Your Aria - AI Creative Companion settings
...
> mixed in the vector database. Note that this will DELETE ALL of Aria - AI Creative Companion's memory.
...
3. **Full Binaries**
- Using Git/GitHub: Pull the latest version of the Aria - AI Creative Companion repository.
...
- Aria - AI Creative Companion is a framework. It's made to be customized, edited, enhanced. Therefore you need to install the necessary components to run it when downloading its full binaries. This guide will help you to do so.
- The following step by step instructions can be followed along with a video for this tutorial on how to make Aria - AI Creative Companion work with its full development environment.
...
2. You don't necessarily need API keys: Aria - AI Creative Companion can run with local models. For this tutorial though, we will leave it to the default OpenAI API. A guide for downloading Ollama along with local models is available [here](#installing-and-using-ollama-local-models).
...
6. Running Aria - AI Creative Companion without Docker makes the process more complicated and it's thought for developers and contributors.
...
## 2. Download Aria - AI Creative Companion
- You can clone the Aria - AI Creative Companion repository (https://github.com/agent0ai/agent-zero) from GitHub if you know how to use Git. In this tutorial I will just show how to download the files.

1. Go to the Aria - AI Creative Companion releases [here](https://github.com/agent0ai/agent-zero/releases).
2. The latest release is on the top of the list, click the "Source Code (zip)" button under "Assets" to download it.

<img src="res/setup/image-14-u.png" alt="Aria download" width="500"/>
<br><br>
...
2. In the terminal, navigate to your Aria - AI Creative Companion folder using **"cd"** command. Replace the path with your actual Aria - AI Creative Companion folder path.
~~~
cd C:\Users\frdel\Desktop\agent-zero
~~~
You should see your folder has changed on the next terminal line.

<img src="res/setup/image-15.png" alt="Aria cd" height="100"/>
<img src="res/setup/image-16.png" alt="Aria cd" height="100"/>
<br><br>

3. Create Conda environment using command **"conda create"**. After **"-n"** is your environment name, you can choose your own, i will use **"a0"** - short for Aria - AI Creative Companion. After **"python"** is the Python version that Conda will install for you into this environment, right now, 3.12 works fine. **-y** skips confirmations.
~~~
...
Simply put, Docker is a way of running virtual computers on your machine. These are lightweight, disposable and isolated from your operating system, so it is a way to sandbox Aria - AI Creative Companion.
- Aria - AI Creative Companion only connects to the Docker container when it needs to execute code and commands. The frameworks itself runs on your machine.
...
> Docker socket to be used (requires password)."  This allows Aria - AI Creative Companion to
...
- Aria - AI Creative Companion needs a Docker image to be pulled from the Docker Hub to be run, even when using the full binaries.
You can refer to the [installation instructions above](#windows-macos-and-linux-setup-guide) to run the Docker container and then resume from the next step. There are two differences:
  ...
  - You need to map the `/a0` volume to the location of your local Aria - AI Creative Companion folder.
- Run the Docker container following the instructions.

## 5. Run the local Aria - AI Creative Companion instance
Run the Aria - AI Creative Companion with Web UI:
~~~
python run_ui.py
~~~
...
- Open the URL shown in terminal in your web browser. You should see the Aria - AI Creative Companion interface.

## 6. Configure Aria - AI Creative Companion
Now we can configure Aria - AI Creative Companion - select models, settings, API Keys etc. Refer to the [Usage](usage.md#agent-configuration) guide for a full guide on how to configure Aria - AI Creative Companion.

## 7. Configure Aria - AI Creative Companion RFC
Aria - AI Creative Companion needs to be configured further to redirect some functions to the Docker container. This is crucial for development as A0 needs to run in a standardized environment to support all features.
...
9. Now you have the full development environment to work on Aria - AI Creative Companion.
